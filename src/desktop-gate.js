// Narrow screens get an honest "open this on a desktop" screen rather than a
// hobbled editor.
//
// The app's chrome is a fixed 488px (312 panel + 128 thumbnail strip + padding),
// so below ~820px the preview is squeezed to nothing — at 390px the stage
// measures 48px and the frame does not render at all. A phone-shaped version
// would also fail at the finish line: a ZIP of ten 2160x2700 PNGs cannot be
// unzipped or posted from iOS, so the user could not use what they made.
//
// The screen is a conversion opportunity, not a dead end: it links to the studio.
...
/**
 * @returns {Promise<void>} resolves when the app may start — either the window
 *   is wide enough, or the visitor chose to continue anyway.
 */
export function requireDesktop({ minWidth = 820, studio = {} } = {}) {
  const wide = () => window.innerWidth >= minWidth;

  let forced = false;
  try { forced = sessionStorage.getItem(KEY) === '1'; } catch { /* private mode */ }
  ...
  return new Promise((resolve) => {
    ...
    // Widening the window is the outcome we actually want - let it through
    // without making them find a button.
